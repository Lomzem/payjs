...
async function fillDate(date, code) {
  const dateCell = await waitForCondition(() =>
    Array.from(document.querySelectorAll("td")).find(
      (div) =>
        div.textContent.includes(date) &&
        Array.from(div.classList).some((c) => c.includes("wide")),
    ),
  );
  ...
  const job = await waitForCondition(() =>
    Array.from(document.querySelectorAll("div.option-list-item")).find((el) =>
      el.innerText.includes(code),
    ),
  );
